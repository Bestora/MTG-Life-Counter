<div x-data="{ 
    ...
    <div class="absolute top-2 lg:top-4 left-3 flex items-center gap-2">
        <flux:dropdown>
            <flux:button size="sm" variant="subtle" icon="ellipsis-horizontal" class="!px-1 !py-1 h-8 w-8 text-zinc-500" />
            <flux:menu>
                <flux:menu.item wire:click="toggleElimination" icon="{{ $player->is_eliminated ? 'heart' : 'no-symbol' }}">
    ...
    <div class="absolute top-2 lg:top-4 right-3 flex items-center gap-1 sm:gap-2">
        @if($isCurrentPlayer)
            <flux:badge size="sm" class="bg-zinc-900 dark:bg-white text-white dark:text-zinc-900 hidden sm:flex">You</flux:badge>
        @endif
        
        <flux:modal.trigger name="counters-{{ $player->id }}">
            <flux:button size="sm" variant="subtle" icon="beaker" class="h-8 w-8 !p-1 text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-200" />
        </flux:modal.trigger>
        
        <flux:modal.trigger name="commander-damage-{{ $player->id }}">
            <flux:button size="sm" variant="subtle" icon="shield-exclamation" class="h-8 w-8 !p-1 text-zinc-500 hover:text-zinc-800 dark:hover:text-zinc-200" />
        </flux:modal.trigger>
    </div>
    
    <!-- Central Life Total with Alpine logic -->
    <div class="text-6xl sm:text-8xl font-black text-zinc-900 dark:text-white tabular-nums select-none tracking-tighter">
    ...
    @if($player->is_eliminated)
        <div class="absolute inset-0 z-10 flex items-center justify-center pointer-events-none">
            <span class="bg-zinc-900/80 text-white font-bold text-xl px-4 py-2 rounded-lg rotate-[-12deg] uppercase tracking-widest shadow-xl">
                Defeated
            </span>
        </div>
    @endif
        ...
        <div class="space-y-6">
            ...
            <div class="space-y-4 max-h-[50vh] overflow-y-auto pb-4 px-1">
                ...
                @foreach($this->otherPlayers as $other)
                    ...
                    @endphp
                    <div class="flex flex-col gap-3 p-3 rounded-xl border border-zinc-200 dark:border-zinc-800" style="border-left: 4px solid {{ $other->color }}">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <div class="flex items-center gap-2">
                                <flux:button wire:click="updateCommanderDamage({{ $other->id }}, -1, false)" icon="minus" size="sm" variant="subtle" class="h-8 w-8 rounded-full" />
                                <span class="text-xl font-bold w-8 text-center tabular-nums">{{ $dmg }}</span>
                                <flux:button wire:click="updateCommanderDamage({{ $other->id }}, 1, false)" icon="plus" size="sm" variant="subtle" class="h-8 w-8 rounded-full" />
                            </div>
                        </div>
                        
                        <!-- Partner Commander -->
                        <div class="flex justify-between items-center opacity-75 pt-2 border-t border-zinc-100 dark:border-zinc-800">
                            <span class="text-sm pl-5">Partner</span>
                            <div class="flex items-center gap-2">
                                <flux:button wire:click="updateCommanderDamage({{ $other->id }}, -1, true)" icon="minus" size="sm" variant="subtle" class="h-7 w-7 rounded-full" />
                                <span class="text-lg font-bold w-8 text-center tabular-nums">{{ $partnerDmg }}</span>
                                <flux:button wire:click="updateCommanderDamage({{ $other->id }}, 1, true)" icon="plus" size="sm" variant="subtle" class="h-7 w-7 rounded-full" />
                            </div>
                        </div>
                    </div>
                @endforeach
            ...
            <div class="space-y-4 max-h-[50vh] overflow-y-auto pb-4 px-1">
                ...
                @foreach($counterTypes as $type)
                    @php
                        $val = $counters[$type] ?? 0;
                    @endphp
                    <div class="flex justify-between items-center p-3 rounded-xl border border-zinc-200 dark:border-zinc-800">
                        <span class="font-medium text-zinc-800 dark:text-zinc-200">{{ $type }}</span>
                        
                        <div class="flex items-center gap-2">
                            <flux:button wire:click="updateCounter('{{ $type }}', -1)" icon="minus" size="sm" variant="subtle" class="h-8 w-8 rounded-full" />
                            <span class="text-xl font-bold w-8 text-center tabular-nums">{{ $val }}</span>
                            <flux:button wire:click="updateCounter('{{ $type }}', 1)" icon="plus" size="sm" variant="subtle" class="h-8 w-8 rounded-full" />
                        </div>
                    </div>
                @endforeach
            </div>
